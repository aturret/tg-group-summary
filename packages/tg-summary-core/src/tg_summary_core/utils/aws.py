...
class AWSClient:
    ...
    def _get_dynamo_items_by_timestamp_range(
        ...
        response = table.query(
            ...
            KeyConditionExpression=Key('chatId').eq(chat_id) & Key('timestamp').between(start_timestamp,
                                                                                        end_timestamp),
    ...
    def _get_presigned_url_for_all_messages(self, group_messages: list) -> list:
        for message in group_messages:
            ...
            if message.get('repliedMessage'):
                if message['repliedMessage'].get('media'):
                    presigned_url = self._get_s3_presigned_url(
