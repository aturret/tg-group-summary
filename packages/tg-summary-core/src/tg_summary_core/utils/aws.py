...
class AWSClient:
    ...
    def _get_dynamo_item_by_key(self, key: dict, table_name: str | None = None) -> dict:
    ...
    def _get_s3_presigned_url(
        ...
        try:
            ...
            self._logger.error(
                "Failed to generate presigned URL for s3://%s/%s: %s", bucket_name, s3_key, e, exc_info=True
            )
            return None

    def _get_dynamo_items_by_timestamp_range(
        self, chat_id: int, start_timestamp: int, end_timestamp: int, table_name: str | None = None
    ) -> list[dict]:
        if table_name is None:
            table_name = settings.dynamo_table_name
        dynamodb = self._session.resource("dynamodb")
